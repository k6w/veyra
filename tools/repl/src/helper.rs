...
impl ReplHelper {
    ...
    pub fn add_variable(&mut self, name: String) { self.completer.add_variable(name); }
    pub fn add_function(&mut self, name: String) { self.completer.add_function(name); }
}

impl Helper for ReplHelper {}
impl Highlighter for ReplHelper {
    fn highlight<'l>(&self, line: &'l str, pos: usize) -> Cow<'l, str> { self.highlighter.highlight(line, pos) }
    fn highlight_char(&self, line: &str, pos: usize, forced: bool) -> bool { self.highlighter.highlight_char(line, pos, forced) }
}
impl Hinter for ReplHelper {
    type Hint = String;
    fn hint(&self, line: &str, pos: usize, ctx: &Context<'_>) -> Option<Self::Hint> {
        self.hinter.hint(line, pos, ctx).filter(|h| !h.is_empty()).map(|h| format!("\x1b[90m{}\x1b[0m", h))
    }
}
impl Completer for ReplHelper {
    type Candidate = Pair;
    fn complete(&self, line: &str, pos: usize, ctx: &Context<'_>) -> rustyline::Result<(usize, Vec<Self::Candidate>)> { self.completer.complete(line, pos, ctx) }
}
impl Validator for ReplHelper { fn validate(&self, ctx: &mut ValidationContext) -> rustyline::Result<ValidationResult> { self.validator.validate(ctx) } }
...
impl CommandCompleter {
    pub fn new(auto_insert_function_parens: bool) -> Self {
        let keywords: HashSet<String> = [
            "let","const","mut","fn","return","if","else","elif","match","case","default","for","while","loop","break","continue","struct","enum","trait","impl","type","use","pub","mod","async","await","defer","try","catch","throw","in","is","as","new","self","super","true","false","None","Some"
        ].iter().map(|s| s.to_string()).collect();
        let commands: HashSet<String> = [
            ":help",":exit",":quit",":clear",":reset",":history",":vars",":funcs",":info",":config",":save",":load",":time",":type",":ast",":tokens",":verbose",":theme",":multiline"
        ].iter().map(|s| s.to_string()).collect();
        Self { keywords, commands, variables: HashSet::new(), functions: HashSet::new(), auto_insert_function_parens }
    }
    pub fn add_variable(&mut self, name: String) { self.variables.insert(name); }
    pub fn add_function(&mut self, name: String) { self.functions.insert(name); }
    fn find_matches(&self, line: &str, pos: usize) -> Vec<Pair> {
        let start = line[..pos].rfind(|c: char| !c.is_alphanumeric() && c!='_' && c!=':').map(|i| i+1).unwrap_or(0);
        let prefix = &line[start..pos];
        if prefix.is_empty() { return vec![]; }
        let mut out = Vec::new();
        if prefix.starts_with(':') {
            for c in &self.commands { if c.starts_with(prefix) { out.push(Self::styled_pair(c,"cmd",c,false)); } }
        } else {
            for k in &self.keywords { if k.starts_with(prefix) { out.push(Self::styled_pair(k,"kw",k,false)); } }
            for v in &self.variables { if v.starts_with(prefix) { out.push(Self::styled_pair(v,"var",v,false)); } }
            for f in &self.functions { if f.starts_with(prefix) { out.push(Self::styled_pair(f,"fn",f,self.auto_insert_function_parens)); } }
        }
        out.sort_by(|a,b| a.display.cmp(&b.display));
        out
    }
    fn styled_pair(raw:&str, kind:&str, replacement:&str, auto_parens:bool)->Pair {
        let (color,label)=match kind {"kw"=>("36","keyword"),"var"=>("33","var"),"fn"=>("35","fn"),"cmd"=>("32","cmd"),_=>("90",kind)};
        let repl = if kind=="fn" && auto_parens { format!("{}()",replacement) } else { replacement.to_string() };

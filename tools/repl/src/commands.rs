...
fn show_history(state: &ReplState, ui: &UI) {
    ...
    for (i, cmd) in history.iter().rev().take(50).enumerate() {
        ...
        println!("{:4} │ {}", 
            ui.theme.muted.paint(num.to_string()),
            cmd
        );
    }
...
fn show_info(state: &ReplState, ui: &UI) {
    ...
    println!("  Syntax highlight:  {}", if config.syntax_highlighting { "ON" } else { "OFF" });
    println!("  Auto-completion:   {}", if config.auto_completion { "ON" } else { "OFF" });
    println!("  Multiline mode:    {}", if config.multiline_mode { "ON" } else { "OFF" });
    println!("  Show timing:       {}", if config.show_timing { "ON" } else { "OFF" });
    println!("  VI mode:           {}", if config.vi_mode { "ON" } else { "OFF" });
    println!("  Auto indent:       {}", if config.auto_indent { "ON" } else { "OFF" });
    println!("  Fancy errors:      {}", if config.fancy_errors { "ON" } else { "OFF" });
...
fn show_config(state: &ReplState, ui: &UI) {
    ...
    table.add_row(vec!["syntax_highlighting".to_string(), config.syntax_highlighting.to_string()]);
    table.add_row(vec!["auto_completion".to_string(), config.auto_completion.to_string()]);
    table.add_row(vec!["multiline_mode".to_string(), config.multiline_mode.to_string()]);
    table.add_row(vec!["show_timing".to_string(), config.show_timing.to_string()]);
    table.add_row(vec!["max_history".to_string(), config.max_history.to_string()]);
    ...
    table.add_row(vec!["auto_indent".to_string(), config.auto_indent.to_string()]);
    table.add_row(vec!["auto_insert_function_parens".to_string(), config.auto_insert_function_parens.to_string()]);
    table.add_row(vec!["fancy_errors".to_string(), config.fancy_errors.to_string()]);
...
fn change_theme(theme_name: &str, state: &mut ReplState, ui: &mut UI) -> Result<()> {
    let new_theme = match theme_name.to_lowercase().as_str() {
        ...
        "solarized-dark" => Some((Theme::solarized_dark(), crate::config::ColorScheme::SolarizedDark)),
        "solarized-light" => Some((Theme::solarized_light(), crate::config::ColorScheme::SolarizedLight)),
        _ => None,
    };

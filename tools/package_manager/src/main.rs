...
impl PackageManager {
    ...
    fn load_project(&self) -> Result<VeyraProject> {
        ...
        if !project_file.exists() {
            return Err(anyhow!("No veyra.toml found. Run 'veyra-pkg init' to create a new project."));
        }
...
"#;
        ...
        println!("{} Created new Veyra project '{}'", "✓".green().bold(), project_name);
    ...
    async fn install_package_locally(&self, package: &str, dev: bool) -> Result<()> {
        ...
        if dev {
            project.dev_dependencies.insert(name.to_string(), version.to_string());
        } else {
            project.dependencies.insert(name.to_string(), version.to_string());
        }
    ...
    async fn download_package(&self, name: &str, version: &str, target_dir: &Path) -> Result<()> {
        ...
        let placeholder_content = format!(r#"# Package: {} v{}
...
"#, name, version, name);
    ...
    async fn build_project(&self, release: bool) -> Result<()> {
        ...
        println!("{} Building project '{}'...", "→".blue().bold(), project.name);
    ...
    async fn run_project(&self, args: Vec<String>) -> Result<()> {
        ...
        println!("{} Running project '{}'...", "→".blue().bold(), project.name);
    ...
    async fn run_tests(&self, filter: Option<String>) -> Result<()> {
        ...
        println!("{} Running tests for '{}'...", "→".blue().bold(), project.name);
        ...
        for test_file in test_files {
            println!("  {} Running {}...", "→".blue(), test_file.file_name().unwrap().to_str().unwrap());
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Install { packages, dev, global } => {
            package_manager.install_packages(packages, dev, global).await?;
        }

...
impl Parser {
    ...
    fn parse_function(&mut self, is_async: bool) -> Result<Function> {
        ...
        let name = self.consume_identifier("Expected function name")?.lexeme.clone();
    ...
    fn parse_parameter(&mut self) -> Result<Parameter> {
        let name = self.consume_identifier("Expected parameter name")?.lexeme.clone();
    ...
    fn parse_struct(&mut self) -> Result<Struct> {
        ...
        let name = self.consume_identifier("Expected struct name")?.lexeme.clone();
    ...
    fn parse_field(&mut self) -> Result<Field> {
        let name = self.consume_identifier("Expected field name")?.lexeme.clone();
    ...
    fn parse_impl(&mut self) -> Result<Impl> {
        ...
        let target = self.consume_identifier("Expected type name")?.lexeme.clone();
    ...
    fn parse_import(&mut self) -> Result<Import> {
        ...
        let mut path = vec![self.consume_identifier("Expected module name")?.lexeme.clone()];
        ...
        while self.match_token(&TokenKind::Dot) {
            if self.check(&TokenKind::LeftBrace) {
                ...
                let mut items = Vec::new();
                loop {
                    items.push(self.consume_identifier("Expected import item")?.lexeme.clone());
                    if !self.match_token(&TokenKind::Comma) {
                        break;
                    }
                }
                ...
                path.push(self.consume_identifier("Expected module name")?.lexeme.clone());
            }
        }
        
        let alias = if self.match_token(&TokenKind::Identifier) && self.previous().lexeme == "as" {
            Some(self.consume_identifier("Expected alias name")?.lexeme.clone())
        } else {
            None
        };
    ...
    fn parse_actor(&mut self) -> Result<Actor> {
        ...
        let name = self.consume_identifier("Expected actor name")?.lexeme.clone();
    ...
    fn parse_statement(&mut self) -> Result<Statement> {
        match &self.peek().kind {
            ...
            TokenKind::LeftBrace | TokenKind::Indent => {
    ...
    fn parse_variable_declaration(&mut self) -> Result<Statement> {
        ...
        let name = self.consume_identifier("Expected variable name")?.lexeme.clone();
    ...
    fn parse_for_statement(&mut self) -> Result<Statement> {
        ...
        let variable = self.consume_identifier("Expected loop variable")?.lexeme.clone();
        ...
        match &self.peek().kind {
            ...
            TokenKind::LeftBrace => {
                ...
                if self.check(&TokenKind::RightBrace) {
                    ...
                    return Ok(Expression::Dictionary(DictionaryExpression { pairs: vec![] }));
                }
                ...
                if self.match_token(&TokenKind::Colon) {
                    ...
                    self.consume(&TokenKind::RightBrace, "Expected '}' after dictionary pairs")?;
    ...
    fn parse_match_expression(&mut self) -> Result<Expression> {
        ...
        while !self.check(&TokenKind::RightBrace) && !self.is_at_end() {
            ...
            let expression = self.parse_expression()?;
            arms.push(MatchExpressionArm { pattern, expression });

...
impl Value {
    fn type_name(&self) -> &'static str {
        match self {
            ...
            Value::Set(_) => "set",
            Value::Tuple(_) => "tuple",
            Value::Reference(r) => if r.mutable { "&mut" } else { "&" },
        }
    }
}
...
impl Environment {
    ...
    fn set(&mut self, name: &str, value: Value) -> Result<()> {
        ...
        Err(VeyraError::runtime_error(format!("Undefined variable '{}'", name)))
    }
}
...
impl Interpreter {
    ...
    fn call_function(&mut self, name: &str, args: &[Value]) -> Result<Value> {
        ...
        match name {
            "print" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("print() takes exactly one argument"));
                }
            ...
            "str" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("str() takes exactly one argument"));
                }
                return Ok(Value::String(self.value_to_string(&args[0])));
            }
            "len" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("len() takes exactly one argument"));
                }
                return match &args[0] {
            ...
            "push" => {
                if args.len() != 2 {
                    return Err(VeyraError::runtime_error("push() takes exactly two arguments"));
                }
                ...
                return Err(VeyraError::runtime_error("push() can only be called on arrays"));
            }
            "pop" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("pop() takes exactly one argument"));
                }
                if let Value::Array(arr) = &args[0] {
                    if arr.is_empty() {
                        return Ok(Value::None);
                ...
                return Err(VeyraError::runtime_error("pop() can only be called on arrays"));
            }
            "type_of" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("type_of() takes exactly one argument"));
                }
                return Ok(Value::String(args[0].type_name().to_string()));
            }
            "int" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("int() takes exactly one argument"));
                }
                return self.cast_value(args[0].clone(), &Type::Primitive(PrimitiveType::I64));
            }
            "float" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("float() takes exactly one argument"));
                }
                return self.cast_value(args[0].clone(), &Type::Primitive(PrimitiveType::F64));
            }
            "bool" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("bool() takes exactly one argument"));
                }
                return self.cast_value(args[0].clone(), &Type::Primitive(PrimitiveType::Bool));
            }
            "char" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("char() takes exactly one argument"));
                }
                return self.cast_value(args[0].clone(), &Type::Primitive(PrimitiveType::Char));
            }
            "is_int" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_int() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::Integer(_))));
            }
            "is_float" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_float() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::Float(_))));
            }
            "is_string" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_string() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::String(_))));
            }
            "is_bool" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_bool() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::Boolean(_))));
            }
            "is_char" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_char() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::Char(_))));
            }
            "is_array" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_array() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::Array(_))));
            }
            "is_dict" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_dict() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::Dictionary(_))));
            }
            "is_none" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("is_none() takes exactly one argument"));
                }
                return Ok(Value::Boolean(matches!(args[0], Value::None)));
            }
            "sqrt" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("sqrt() takes exactly one argument"));
                }
                match &args[0] {
            ...
            "pow" => {
                if args.len() != 2 {
                    return Err(VeyraError::runtime_error("pow() takes exactly two arguments"));
                }
                return self.apply_binary_operator(&BinaryOperator::Power, &args[0], &args[1]);
            }
            "abs" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("abs() takes exactly one argument"));
                }
                match &args[0] {
            ...
            "min" => {
                if args.len() != 2 {
                    return Err(VeyraError::runtime_error("min() takes exactly two arguments"));
                }
                match (&args[0], &args[1]) {
                    (Value::Integer(a), Value::Integer(b)) => return Ok(Value::Integer(*a.min(b))),
                    (Value::Float(a), Value::Float(b)) => return Ok(Value::Float(a.min(*b))),
                    (Value::Integer(a), Value::Float(b)) => return Ok(Value::Float((*a as f64).min(*b))),
                    (Value::Float(a), Value::Integer(b)) => return Ok(Value::Float(a.min(*b as f64))),
                    _ => return Err(VeyraError::runtime_error("min() requires numbers")),
                }
            }
            "max" => {
                if args.len() != 2 {
                    return Err(VeyraError::runtime_error("max() takes exactly two arguments"));
                }
                match (&args[0], &args[1]) {
                    (Value::Integer(a), Value::Integer(b)) => return Ok(Value::Integer(*a.max(b))),
                    (Value::Float(a), Value::Float(b)) => return Ok(Value::Float(a.max(*b))),
                    (Value::Integer(a), Value::Float(b)) => return Ok(Value::Float((*a as f64).max(*b))),
                    (Value::Float(a), Value::Integer(b)) => return Ok(Value::Float(a.max(*b as f64))),
                    _ => return Err(VeyraError::runtime_error("max() requires numbers")),
                }
            }
            "floor" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("floor() takes exactly one argument"));
                }
                match &args[0] {
            ...
            "ceil" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("ceil() takes exactly one argument"));
                }
                match &args[0] {
            ...
            "round" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("round() takes exactly one argument"));
                }
                match &args[0] {
            ...
            "string_to_upper" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("string_to_upper() takes exactly one argument"));
                }
                match &args[0] {
                    Value::String(s) => return Ok(Value::String(s.to_uppercase())),
                    _ => return Err(VeyraError::runtime_error("string_to_upper() requires a string")),
                }
            }
            "string_to_lower" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("string_to_lower() takes exactly one argument"));
                }
                match &args[0] {
                    Value::String(s) => return Ok(Value::String(s.to_lowercase())),
                    _ => return Err(VeyraError::runtime_error("string_to_lower() requires a string")),
                }
            }
            "string_trim" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("string_trim() takes exactly one argument"));
                }
                match &args[0] {
            ...
            "clamp" => {
                if args.len() != 3 {
                    return Err(VeyraError::runtime_error("clamp() takes exactly three arguments: value, min, max"));
                }
                match (&args[0], &args[1], &args[2]) {
                    ...
                    _ => return Err(VeyraError::runtime_error("clamp() requires numeric arguments")),
                }
            }
            "array_sum" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("array_sum() takes exactly one argument"));
                }
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        for val in arr {
                            match val {
                                ...
                                _ => return Err(VeyraError::runtime_error("array_sum() requires an array of numbers")),
                            }
                        }
            ...
            "array_avg" => {
                if args.len() != 1 {
                    return Err(VeyraError::runtime_error("array_avg() takes exactly one argument"));
                }
                match &args[0] {
                    Value::Array(arr) => {
                        if arr.is_empty() {
                        ...
                        for val in arr {
                            match val {
                                ...
                                Value::Float(f) => sum += f,
                                _ => return Err(VeyraError::runtime_error("array_avg() requires an array of numbers")),
                            }
                        }
            ...
            "range" => {
                if args.len() < 1 || args.len() > 3 {
                    return Err(VeyraError::runtime_error("range() takes 1, 2, or 3 arguments"));
                }
                ...
                let (start, end, step) = match args.len() {
                    1 => {
                        match &args[0] {
                            ...
                            _ => return Err(VeyraError::runtime_error("range() requires integer arguments")),
                        }
                    }
                    2 => {
                        match (&args[0], &args[1]) {
                    ...
                    3 => {
                        match (&args[0], &args[1], &args[2]) {
                            (Value::Integer(s), Value::Integer(e), Value::Integer(step)) => (*s, *e, *step),
                            _ => return Err(VeyraError::runtime_error("range() requires integer arguments")),
                        }
                    }
                    _ => unreachable!(),
    ...
    fn call_module_function(&mut self, name: &str, _args: &[Value]) -> Result<Value> {
        ...
        Err(VeyraError::runtime_error(format!("Undefined function '{}'", name)))
    }
        ...
        match statement {
                    ...
                    Expression::Identifier(name) => {
                        match assignment.operator {
                            ...
                            AssignmentOperator::AddAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                            ...
                            AssignmentOperator::SubAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::Subtract, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::MulAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::Multiply, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::DivAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::Divide, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::ModAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::Modulo, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::BitwiseAndAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::BitwiseAnd, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::BitwiseOrAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::BitwiseOr, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::BitwiseXorAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::BitwiseXor, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::LeftShiftAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::LeftShift, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                            AssignmentOperator::RightShiftAssign => {
                                let old_value = self.environment.get(name)
                                    .ok_or_else(|| VeyraError::runtime_error(format!("Undefined variable '{}'", name)))?
                                    .clone();
                                let new_value = self.apply_binary_operator(&BinaryOperator::RightShift, &old_value, &value)?;
                                self.environment.set(name, new_value)?;
                            }
                        }
                    }
                    _ => {
                        return Err(VeyraError::runtime_error("Complex assignment targets not implemented yet"));
                    }
                }
            }
            ...
            Statement::While(while_stmt) => {
                while self.evaluate_expression(&while_stmt.condition)?.is_truthy() {
                    match self.execute_block(&while_stmt.body) {
                        ...
                        Err(VeyraError::RuntimeError { message }) if message == "continue" => continue,
                        Err(e) => return Err(e),
                    }
                }
            }
            Statement::For(for_stmt) => {
                ...
                match iterable {
                    ...
                    _ => return Err(VeyraError::runtime_error("Cannot iterate over non-array value")),
                }
            }
            Statement::Return(return_stmt) => {
                ...
                return Err(VeyraError::runtime_error(format!("return:{}", self.value_to_string(&value))));
            }
            Statement::Break => {
            ...
            Statement::Match(_) => {
                return Err(VeyraError::runtime_error("match statements not implemented"));
            }
        }
        Ok(())
    }
    ...
    fn evaluate_expression(&mut self, expression: &Expression) -> Result<Value> {
        match expression {
            ...
            Expression::Identifier(name) => {
                if name == "self" {
                    return Err(VeyraError::runtime_error("'self' not supported in this context"));
                }
            ...
            Expression::Call(call) => {
                ...
                match call.callee.as_ref() {
                    ...
                    _ => Err(VeyraError::runtime_error("Complex function calls not implemented"))
                }
            }
            ...
            Expression::Dictionary(dict) => {
                ...
                for (key_expr, value_expr) in &dict.pairs {
                    ...
                    let key_str = match key {
                        ...
                        _ => return Err(VeyraError::runtime_error("Dictionary keys must be hashable (string, int, float, bool, or char)")),
                    };
            ...
            Expression::Set(set) => {
                ...
                for elem_expr in &set.elements {
                    ...
                    let elem_str = match elem {
                        ...
                        Value::Boolean(b) => b.to_string(),
                        Value::Char(c) => c.to_string(),
                        _ => return Err(VeyraError::runtime_error("Set elements must be hashable (string, int, float, bool, or char)")),
                    };
            ...
            Expression::Index(index) => {
                ...
                match (object, index_val) {
                    ...
                    (Value::Dictionary(map), Value::String(key)) => {
                        map.get(&key)
                            .cloned()
                            .ok_or_else(|| VeyraError::runtime_error(format!("Key '{}' not found in dictionary", key)))
                    }
            ...
            Expression::Range(range) => {
                ...
                if let (Value::Integer(s), Value::Integer(e)) = (start, end) {
                    ...
                    Err(VeyraError::runtime_error("Range expressions require integer bounds"))
                }
            }
            Expression::ModuleAccess(module_access) => {
                ...
                match full_name.as_str() {
                    ...
                    _ => {
                        ...
                        self.environment.get(&module_access.item)
                            .cloned()
                            .ok_or_else(|| VeyraError::runtime_error(format!("Undefined module item '{}::{}'", module_access.module, module_access.item)))
                    }
                }
            }
    ...
    fn apply_binary_operator(&self, op: &BinaryOperator, left: &Value, right: &Value) -> Result<Value> {
        match (op, left, right) {
            // Arithmetic
            ...
            (BinaryOperator::Add, Value::Integer(a), Value::Float(b)) => Ok(Value::Float(*a as f64 + b)),
            (BinaryOperator::Add, Value::Float(a), Value::Integer(b)) => Ok(Value::Float(a + *b as f64)),
            ...
            (BinaryOperator::Subtract, Value::Integer(a), Value::Float(b)) => Ok(Value::Float(*a as f64 - b)),
            (BinaryOperator::Subtract, Value::Float(a), Value::Integer(b)) => Ok(Value::Float(a - *b as f64)),
            ...
            (BinaryOperator::Multiply, Value::Integer(a), Value::Float(b)) => Ok(Value::Float(*a as f64 * b)),
            (BinaryOperator::Multiply, Value::Float(a), Value::Integer(b)) => Ok(Value::Float(a * *b as f64)),
            ...
            (BinaryOperator::Less, Value::String(a), Value::Char(b)) => Ok(Value::Boolean(*a < b.to_string())),
            (BinaryOperator::Less, Value::Char(a), Value::String(b)) => Ok(Value::Boolean(a.to_string() < *b)),
            ...
            (BinaryOperator::LessEqual, Value::String(a), Value::String(b)) => Ok(Value::Boolean(a <= b)),
            (BinaryOperator::LessEqual, Value::Char(a), Value::Char(b)) => Ok(Value::Boolean(a <= b)),
            (BinaryOperator::LessEqual, Value::String(a), Value::Char(b)) => Ok(Value::Boolean(*a <= b.to_string())),
            (BinaryOperator::LessEqual, Value::Char(a), Value::String(b)) => Ok(Value::Boolean(a.to_string() <= *b)),
            ...
            (BinaryOperator::Greater, Value::String(a), Value::String(b)) => Ok(Value::Boolean(a > b)),
            (BinaryOperator::Greater, Value::Char(a), Value::Char(b)) => Ok(Value::Boolean(a > b)),
            (BinaryOperator::Greater, Value::String(a), Value::Char(b)) => Ok(Value::Boolean(*a > b.to_string())),
            (BinaryOperator::Greater, Value::Char(a), Value::String(b)) => Ok(Value::Boolean(a.to_string() > *b)),
            ...
            (BinaryOperator::GreaterEqual, Value::String(a), Value::String(b)) => Ok(Value::Boolean(a >= b)),
            (BinaryOperator::GreaterEqual, Value::Char(a), Value::Char(b)) => Ok(Value::Boolean(a >= b)),
            (BinaryOperator::GreaterEqual, Value::String(a), Value::Char(b)) => Ok(Value::Boolean(*a >= b.to_string())),
            (BinaryOperator::GreaterEqual, Value::Char(a), Value::String(b)) => Ok(Value::Boolean(a.to_string() >= *b)),
            ...
            (BinaryOperator::BitwiseAnd, Value::Integer(a), Value::Integer(b)) => Ok(Value::Integer(a & b)),
            (BinaryOperator::BitwiseOr, Value::Integer(a), Value::Integer(b)) => Ok(Value::Integer(a | b)),
            (BinaryOperator::BitwiseXor, Value::Integer(a), Value::Integer(b)) => Ok(Value::Integer(a ^ b)),
    ...
    fn cast_value(&self, value: Value, target_type: &Type) -> Result<Value> {
        match target_type {
            Type::Primitive(prim_type) => {
                match prim_type {
                    PrimitiveType::Int | PrimitiveType::I32 | PrimitiveType::I64 => {
                        match value {
                            ...
                            Value::String(s) => {
                                s.parse::<i64>()
                                    .map(Value::Integer)
                                    .map_err(|_| VeyraError::runtime_error(format!("Cannot cast '{}' to int", s)))
                            }
                    ...
                    PrimitiveType::F32 | PrimitiveType::F64 => {
                        match value {
                            ...
                            Value::String(s) => {
                                s.parse::<f64>()
                                    .map(Value::Float)
                                    .map_err(|_| VeyraError::runtime_error(format!("Cannot cast '{}' to float", s)))
                            }
                            _ => Err(VeyraError::runtime_error(format!(
                            ...
                            Value::Integer(n) => {
                                if n >= 0 && n <= 0x10FFFF {
                                    std::char::from_u32(n as u32)
                                        .map(Value::Char)
                                        .ok_or_else(|| VeyraError::runtime_error(format!("Invalid Unicode code point: {}", n)))
                                } else {
                                    Err(VeyraError::runtime_error(format!("Invalid Unicode code point: {}", n)))
                                }
                            }
                            Value::String(s) => {
                                if s.len() == 1 {
                                    ...
                                    Err(VeyraError::runtime_error(format!("Cannot cast string of length {} to char", s.len())))
                                }
                            }
                            _ => Err(VeyraError::runtime_error(format!(
                                "Cannot cast {} to char",
    ...
    fn load_math_module(&mut self) -> Result<()> {
        ...
        self.environment.define("PI".to_string(), Value::Float(std::f64::consts::PI));
        self.environment.define("E".to_string(), Value::Float(std::f64::consts::E));
...
impl Interpreter {
    ...
    fn _load_stdlib_file(&mut self, path: &str) -> Result<()> {
        ...
        let project_root = exe_dir.parent() // up from debug
        ...
        let source = fs::read_to_string(&stdlib_path)
            .map_err(|e| VeyraError::IoError(format!("Failed to load stdlib file '{}': {}", stdlib_path.display(), e)))?;
